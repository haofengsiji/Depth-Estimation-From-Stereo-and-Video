...
tic
 ...
 for w = 1:width
     for h = 1:height
         ...
         % data term
         w_ls = D+w;
         match_w_ls = w_ls(1,find(w_ls>=1,1):find(w_ls<=450,1,'last'));
         match_cost = sum(abs(node1 - node2s(match_w_ls,:)),2);
         cover_start = find(w_ls>=1,1);
         cover_end = find(w_ls<=450,1,'last');
...
tic
[labels,E,EA] = GCMex(class-1,single(unary),pairwise*5,single(labelcost),1);
toc
